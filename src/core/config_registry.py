...
_FIELD_DEFINITIONS: Dict[str, Dict[str, Any]] = {
    ...
    "JEXI_ENABLED": {
        ...
        "description": "Enable the J.E.X.I. multi-agent layer (persona registry, boss agent, self-loop). Off by default.",
        "category": "agent",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "JEXI_NTFY_URL": {
        "title": "Jexi ntfy URL",
        "description": "ntfy publish endpoint for Jexi reports (overrides NTFY_URL). e.g. https://ntfy.sh/jexi_reports",
        "category": "agent",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "JEXI_NTFY_SERVER": {
        "title": "Jexi ntfy Server",
        "description": "ntfy server root used when only JEXI_NTFY_TOPIC is set (default https://ntfy.sh).",
        "category": "agent",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "JEXI_NTFY_TOPIC": {
        "title": "Jexi ntfy Topic",
        "description": "Default topic when no JEXI_NTFY_URL is configured (default jexi_reports).",
        "category": "agent",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "jexi_reports",
        "options": [],
        "validation": {},
        "display_order": 22,
    },
    "JEXI_NTFY_TOKEN": {
        "title": "Jexi ntfy Token",
        "description": "Optional Bearer Token for a protected ntfy topic.",
        "category": "agent",
        "data_type": "string",
        "ui_control": "password",
        "is_sensitive": True,
    ...
    "JEXI_MAX_ITERATIONS": {
        "title": "Jexi Max Iterations",
        "description": "Cap for the self-improvement loop (test -> evaluate -> adapt -> re-test).",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
        ...
        "default_value": "10",
        "options": [],
        "validation": {"item_type": "integer", "min": 1, "max": 1000},
        "display_order": 30,
    },
    "JEXI_TARGET_WIN_RATE": {
        "title": "Jexi Target Win Rate",
        "description": "Self-loop convergence target: fraction of trades with positive pnl.",
        "category": "agent",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
        ...
        "default_value": "0.55",
        "options": [],
        "validation": {"item_type": "float", "min": 0.0, "max": 1.0},
        "display_order": 31,
    },
    "JEXI_TARGET_SHARPE": {
        "title": "Jexi Target Sharpe",
        "description": "Self-loop convergence target: annualised Sharpe on daily equity curve.",
        "category": "agent",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "JEXI_TARGET_MAX_DRAWDOWN": {
        "title": "Jexi Target Max Drawdown",
        "description": "Self-loop convergence target: maximum allowed peak-to-trough drawdown.",
        "category": "agent",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "JEXI_TARGET_MIN_TOTAL_RETURN": {
        "title": "Jexi Target Min Return",
        "description": "Self-loop convergence target: minimum total return per scenario window.",
        "category": "agent",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "JEXI_RISK_PER_TRADE": {
        "title": "Jexi Risk per Trade",
        "description": "Max fraction of equity risked to the stop-loss on any single trade (default 0.02).",
        "category": "agent",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "JEXI_MAX_POSITION_FRACTION": {
        "title": "Jexi Max Position",
        "description": "Hard cap on any single position as a fraction of equity (default 0.25).",
        "category": "agent",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "JEXI_MAX_DRAWDOWN_HALT": {
        "title": "Jexi Drawdown Halt",
        "description": "Fail-safe: pause new paper trades if portfolio drawdown exceeds this fraction (default 0.10).",
        "category": "agent",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "JEXI_SPECIALISTS": {
        "title": "Jexi Active Specialists",
        "description": "Comma-separated specialist ids to dispatch (empty = all 50).",
        "category": "agent",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "JEXI_RESEARCH_BACKEND": {
        "title": "Jexi Research Backend",
        "description": "Real LLM research for specialists: off (deterministic fallback), auto (opencode_cli if installed else litellm if configured), opencode_cli (built-in, no key), or litellm (needs repo LLM keys).",
        "category": "agent",
        "data_type": "string",
        "ui_control": "select",
        "is_sensitive": False,
        ...
        "is_editable": True,
        "default_value": "off",
        "options": ["off", "auto", "opencode_cli", "litellm"],
        "validation": {},
        "display_order": 44,
    },
    "JEXI_REGIME_GATE": {
        "title": "Jexi Regime Gate",
        "description": "Gate consensus trades on Prof Thorne's market state: no fresh longs in a bear tape, no fresh shorts in a bull tape, only strong-conviction trades sideways. Off by default to preserve 2022-and-earlier behavior.",
        "category": "agent",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "JEXI_MCP_SERVERS": {
        "title": "Jexi MCP Servers (JSON)",
        "description": "JSON array of MCP stdio servers: [{\"name\":\"finance-data-mcp\",\"command\":\"npx\",\"args\":[\"-y\",\"finance-data-mcp\"],\"env\":{}}]",
        "category": "agent",
        "data_type": "json",
        "ui_control": "textarea",
        "is_sensitive": False,
    ...
    "JEXI_REPORT_INTERVAL_HOURS": {
        "title": "Jexi Report Interval (hours)",
        "description": "How often the scheduler should push a Jexi report (default 1; wiring owned by ops).",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
        ...
        "default_value": "1",
        "options": [],
        "validation": {"item_type": "integer", "min": 1, "max": 168},
        "display_order": 60,
    },
    "GOTIFY_URL": {
        "title": "Gotify URL",
...
_FIELD_HELP_METADATA: Dict[str, Dict[str, Any]] = {
    ...
    "JEXI_ENABLED": {
        ...
        "examples": ["JEXI_ENABLED=true"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_NTFY_URL": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_NTFY_URL=https://ntfy.sh/jexi_reports"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_NTFY_SERVER": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_NTFY_SERVER=https://ntfy.sh"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_NTFY_TOPIC": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_NTFY_TOPIC=jexi_reports"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_NTFY_TOKEN": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_NTFY_TOKEN=your_ntfy_token"],
        "docs": _DOC_JEXI,
        "warning_codes": ["secret_value"],
    },
    "JEXI_MAX_ITERATIONS": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_MAX_ITERATIONS=10"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_TARGET_WIN_RATE": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_TARGET_WIN_RATE=0.55"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_TARGET_SHARPE": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_TARGET_SHARPE=1.2"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_TARGET_MAX_DRAWDOWN": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_TARGET_MAX_DRAWDOWN=0.15"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_TARGET_MIN_TOTAL_RETURN": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_TARGET_MIN_TOTAL_RETURN=0.05"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_RISK_PER_TRADE": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_RISK_PER_TRADE=0.02"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_MAX_POSITION_FRACTION": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_MAX_POSITION_FRACTION=0.25"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_MAX_DRAWDOWN_HALT": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_MAX_DRAWDOWN_HALT=0.10"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_SPECIALISTS": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_SPECIALISTS=vix,mac,thorn"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_RESEARCH_BACKEND": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_RESEARCH_BACKEND=auto", "JEXI_RESEARCH_BACKEND=opencode_cli"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_REGIME_GATE": {
    ...
    "JEXI_MCP_ENABLED": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_MCP_ENABLED=false"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_MCP_SERVERS": {
        "help_key": "settings.jexi",
        "examples": ['JEXI_MCP_SERVERS=[{"name":"finance-data-mcp","command":"npx","args":["-y","finance-data-mcp"]}]'],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "JEXI_REPORT_INTERVAL_HOURS": {
        "help_key": "settings.jexi",
        "examples": ["JEXI_REPORT_INTERVAL_HOURS=1"],
        "docs": _DOC_JEXI,
        "warning_codes": [],
    },
    "GOTIFY_URL": {
