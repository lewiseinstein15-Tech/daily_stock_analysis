...
## [0.8.0] — Real agent stack + terms gate + live-money readiness
- [Added] **Bull/bear debate** (`jexi_market/agents/debate.py`) — the TradingAgents pattern built native: bull and bear cases assembled from real agent votes, factors and research (fundamentals + news), one rebuttal round with evidence-based counters, research-manager verdict. Winner moves confidence by at most ±0.10; every decision now carries labelled [bull]/[bear] evidence.
- [Added] **Risk team** (`jexi_market/agents/risk_team.py`) — aggressive/neutral/conservative reviewers score the actual trade against actual numbers (drawdown, daily P&L, ATR%, position size, earnings proximity); neutral+conservative rejection = PM veto (fraction zeroed); 1-2 rejections trim size to 75%/50%. The hard RiskGate still runs after and remains the only enforcing authority.
- [Added] **LLM arbiter** (`jexi_market/agents/llm_arbiter.py`) — the AI key saved in the user's Jexi account (OpenAI/Gemini/Anthropic/DeepSeek) finally gets used: it judges the full evidence pack (factors + votes + debate) and returns a strict JSON verdict whose influence is clamped to ±0.15 confidence. Malformed/slow/absent = no opinion; the quantitative pipeline never depends on it. `JEXI_LLM_ARBITER=auto|off`, `JEXI_LLM_MODEL` to pin a model.
- [Added] **Skills pack** (`jexi_market/skills/`) — five deterministic skills: earnings proximity (yfinance calendar), volatility regime (ATR + squeeze), liquidity (dollar volume), gap risk (overnight gap history), correlation guard (same-sector holdings). Skills emit evidence + risk flags + a size hint folded into the decision.
- [Added] **Reflection** (`PerformanceMemory.reflect()`, FinMem pattern) — deterministic lessons from closed trades (drifting agents, regime bias, losing streaks, stop-breach exits) now appear in the morning plan notes and the app feed.
- [Added] **Terms & Policies gate** — every user must accept the current version: `users.terms_version/terms_accepted_at` (D1 migration), `POST/GET /api/account/terms`, terms fields on login/register/me; the web app blocks all views behind an accept modal (real-risk language, links to the full documents, decline = sign out). All app views now require sign-in.
- [Changed] `.env.example` documents the vetted community MCP servers (tradingview-mcp, FinanceMCP, yahoo-finance2, metatrader-mcp-server) for optional use via the existing MCP client.
- [Tests] 38 new tests (`tests/jexi_market/test_v05_agents.py`); jexi_market suite 270 passed.
...
- [新功能] Prof Thorne 市场状态**方向门槛**（`--regime-gate` / `JEXI_REGIME_GATE`，默认关）：熊市不放弱多头、牛市不放弱空头、震荡只留强信心持仓，用作回测/循环的可选方向过滤。
- [新功能] J.E.X.I. specialist 支持真实 LLM 研究模式（`src/jexi/research.py`）：`JEXI_RESEARCH_BACKEND`（`off|auto|opencode_cli|litellm`，默认 `off`）与 `--research` CLI 选项；`opencode_cli` 使用内置 opencode CLI（无需 API key），`litellm` 复用仓库 `src/llm` 生成后端（需配置模型/key），`auto` 按可用性降级，任何失败自动回退确定性因子引擎；模型回应（direction/conviction/rationale/risk）按 35/65 与因子分数融合，研究调用按 `(bucket, code, persona)` 缓存（滚动回测每场景每标的每 persona 一次）。
- [新功能] J.E.X.I. 支持 2020-2025 五年滚动纸面回测（`--mode backtest` 与 `--rebalance-days` 滚动自改进循环）：每 `rebalance_days` 个交易日基于前 `lookback_days` 根 K 线重建共识并 papersim 交易，权益跨 chunk 复利，输出 bull/bear/sideways/volatile 各场景与全 5 年（`2020-01-01..2025-12-31`）的胜率/Sharpe/最大回撤/总收益；信号切片恒止于 chunk 起点之前，无未来函数。
- [测试] 新增研究模式与滚动回测离线测试：backend 解析默认 off、JSON 观点解析与拒绝、研究融合/失败回退、缓存命中、窗口边界无未来函数与空窗口零指标。
- [新功能] 新增 J.E.X.I. 多智能体增强层 pilot（`src/jexi/` + `scripts/jexi_cli.py`）：Jexi boss 编排器、Prof Thorne 市场状态判定、Vic Sterling 执行计划、50 位 specialist persona（`src/jexi/personas.yaml`）驱动的确定性分析运行时、长/短双向历史纸面模拟（`papersim.py`，止损/止盈成对、单笔风险 ≤2%、仓位上限、高波动对半降仓）与 win-rate/Sharpe/max drawdown/总收益四指标自改进循环（`loop.py`）；报告经 ntfy.sh JSON API 推送（默认 topic `jexi_reports`，未配置时不发网只写日志），可选 MCP 数据源适配（`mcp_client.py`，`JEXI_MCP_ENABLED`，默认关闭）。
- [新功能] 新增 `JEXI_*` 配置族（`src/config.py` `Config` 字段与 getenv 映射、`config_registry.py` 注册 + `jexi` 分类与 web 帮助文案、`.env.example` 示例）：`JEXI_ENABLED`、`JEXI_NTFY_URL/SERVER/TOPIC/TOKEN`、`JEXI_MAX_ITERATIONS`、`JEXI_TARGET_WIN_RATE/SHARPE/MAX_DRAWDOWN/MIN_TOTAL_RETURN`、`JEXI_RISK_PER_TRADE/MAX_POSITION_FRACTION/MAX_DRAWDOWN_HALT`、`JEXI_SPECIALISTS`、`JEXI_MCP_ENABLED/SERVERS`、`JEXI_REPORT_INTERVAL_HOURS`；默认关闭，不影响现有分析流程（不配置也可运行）。
- [测试] 新增离线 Jexi 测试（`tests/test_jexi_pilot.py`）：persona 注册表加载与校验、指标数学、合成行情下 specialist 方向性、长/短 papersim、Regime 分类、仓位约束、端到端自改进循环（合成数据、权重越界护栏）。
